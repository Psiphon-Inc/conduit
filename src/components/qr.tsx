...
const AnimatedModule: React.FC<{
    module: AnimatedModule;
    color: string;
}> = ({ module, color }) => {
    const translateX = useSharedValue(module.startX);
    const translateY = useSharedValue(module.startY);

    React.useEffect(() => {
        translateX.value = withSpring(module.finalX, {
            damping: 15,
            stiffness: 100,
        });
        translateY.value = withSpring(module.finalY, {
            damping: 15,
            stiffness: 100,
        });
    }, [module, translateX, translateY]);
    ...
    return (
        <Animated.View style={[{ position: "absolute" }, animatedStyle]}>
            <Canvas style={{ width: module.width, height: module.height }}>
                <Rect
                    x={0}
                    y={0}
                    width={module.width}
...
export const QRCode: React.FC<QRCodeProps> = ({
    ...
    const animatedModules = React.useMemo(() => {
        const result: AnimatedModule[] = [];

        for (let row = 0; row < moduleCount; row++) {
            for (let col = 0; col < moduleCount; col++) {
                if (qr.isDark(row, col)) {
                    const finalX = col * moduleSize;
                    const finalY = row * moduleSize;
        ...
        return result;
    }, [qr, moduleCount, moduleSize, size]);
    ...
    return (
        <Animated.View style={{ width: size, height: size }}>
            ...
            {animatedModules.map((module, index) => (
                <AnimatedModule
                    key={index}
                    module={module}
