...
describe("hosted API client contracts", () => {
    ...
    it("creates a billing portal session with bearer auth", async () => {
        ...
        await expect(
            client.createBillingPortalSession("access-token"),
        ).resolves.toEqual({
            url: "https://billing.revenuecat.com/app/sub?token=abc",
        });

        expect(fetchImpl).toHaveBeenCalledWith(
            "https://hcb.example.test/v1/account/billing-portal",
            expect.objectContaining({
                method: "POST",
                headers: expect.objectContaining({
                    authorization: "Bearer access-token",
    ...
    it("rejects non-HTTPS billing portal sessions", async () => {
        const fetchImpl = jest.fn().mockResolvedValue({
            ok: true,
            status: 200,
            text: async () =>
                JSON.stringify({
                    url: "http://billing.revenuecat.com/app/sub?token=abc",
                }),
        });
        const client = createHostedApiClient({
            baseUrl: "https://hcb.example.test",
        ...
        await expect(
            client.createBillingPortalSession("access-token"),
        ).rejects.toThrow("Billing portal URL must use HTTPS");
    });
